...
- Personal skills in rulesync: `git-commit-flow` (group unstaged work into focused commits, issue-linked when required); `force-pushing-ghes-default` (GH003 on github.intuit.com fork default branches)
...
- CWS sets `PLANNOTATOR_REMOTE=1` and `PLANNOTATOR_PORT=19432` so the laptop can SSH-tunnel the Plannotator web UI. That is not a public share. `plannotator annotate` still opens a browser. In Herdr, review markdown with `plannotator/herdr-annotate` (`prefix+f` folder, or the Markdown file matching the terminal selection; `prefix+ctrl+y` last reply of the focused pane's agent — Cursor transcripts included, because upstream tui treats unknown hosts as Claude; `prefix+u` capture). Needs Bun. Laptop Herdr sets `copy_on_select = false` so capture and selection-open can read the mouse selection.
- Bootstrap pins `persiyanov/herdr-reviewr` for git diffs beside the agent (`prefix+shift+f` toggles the pane). Default scope is branch vs trunk. `j`/`k` pick files; `c` comments; `s` sends to the agent; `m` previews markdown. Plugin config: `herdr/reviewr/config.toml`.
...
6. In this dotfiles repository only, direct user-requested maintenance may be committed without an issue number when no issue exists; use a conventional commit subject such as `chore(skills): update shared tooling`
