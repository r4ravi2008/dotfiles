...
- Hunk (`hunk`) and Plannotator (`plannotator`) are CLI tools from `packages.conf`. Bootstrap also installs Hunk's review skill and Plannotator extras (`compound`, `setup-goal`, `visual-explainer`). Sharing is off (`PLANNOTATOR_SHARE=disabled` and `~/.plannotator/config.json`). CWS sets `PLANNOTATOR_REMOTE=1` and `PLANNOTATOR_PORT=19432` only so the laptop can SSH-tunnel the local UI. That is not a public share. Bootstrap also pins Herdr plugins `dleen.herdr-agents` (`prefix+a`), `official.browser`, and `official.plannotator`, builds/links the always-on pane HUD plugin `herdr-pane-minimap` (Kitty layer, not a popup), and sets `kitty_graphics`. Tmux `pane-minimap` popup is unchanged. Native agent keys: previous/next `alt+shift+[` / `alt+shift+]`, focus `prefix+alt+1..9`. Configure runs after the Herdr server is up (CWS starts it in bootstrap) and only when Bun and Chrome/Chromium exist; otherwise bootstrap warns and continues. Sharing stays `PLANNOTATOR_SHARE=disabled`.
