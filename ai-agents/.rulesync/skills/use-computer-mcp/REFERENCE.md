...
When required semantic text ends in `...`, request a bounded limit first (`text_limit: 1000`). Use `"text_limit":"max"` only when the complete text is required. For an incomplete visible long page, list, table, or web app, increase the tree budget after scrolling has failed to expose it (`max_tree_nodes: 3000`, `max_tree_depth: 96`).
