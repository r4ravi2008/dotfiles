...
description: >-
  ...
  cloudworkspaces-run_workspace_command, skip tags, skip herdr machine add,
  herdr --remote, HERDR_ENV unset, or inline create_workspace instead of
  creating-cws-from-devstack-fork.
---

# Developing in CWS

Read the Jira ticket. Let `creating-cws-from-devstack-fork` get a tagged fork CWS and a Herdr saved machine with a pane on it. Then create `<jira-id>-<concise-name>` branches in that workspace.

**REQUIRED SUB-SKILL:** Use `creating-cws-from-devstack-fork` for every provision, start, tag, wait, `herdr machine add`, and opening a pane on that machine. Do not inline those steps here. Do not use IDDA or Computer Use.

**REQUIRED SUB-SKILL:** Use `herdr` after attach.

Do not SSH-forward laptop ports 8787, 3118, or 19432.
...
| "`herdr --remote` / unset HERDR_ENV" | Attach is `herdr machine add` plus a pane on that machine. |
| "Open in Cursor is one click" | Skips remote keybindings. Use `herdr machine add`. |
...
- Inlined `create_workspace` / `start_workspace` / `herdr machine add` / `herdr --remote` in this skill
