...
Put the host on this Herdr client with Herdr Mirror. CWS sshd does not forward unix sockets.

```toml
close_remote_on_local_close = false
default_host = "<short-id>"

[hosts.<short-id>]
target = "cws.devstack-<id>"
prefix = "<short-id>"
api_transport = "exec"
```
...
herdr-mirror start
herdr-mirror status
herdr workspace list   # expect "<prefix>: ..."
```

Config path is `~/.config/herdr-mirror/hosts.toml`. Disable stale CWS hosts with `enabled = false`. Do not `herdr-mirror teardown`. Do not `herdr machine add` for CWS attach. Drive: `developing-in-cws` `control-plane.md`.
...
Isolated probe (does not steal laptop 8787; wait and bootstrap checklist only, not Herdr drive):
